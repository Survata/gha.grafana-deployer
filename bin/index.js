...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.runDeployment = void 0;
const git_1 = __nccwpck_require__(2);
const grafana_1 = __nccwpck_require__(907);
const util_1 = __nccwpck_require__(744);
...
function deployFolder(sourcePath, folder) {
    console.log('Deploying folder', folder);
    let folderId = grafana_1.grafana.getFolderId(folder);
...
function deployDashboard(sourcePath, folder, folderId, file) {
    ...
    const grafanaDashboard = grafana_1.grafana.getDashboard(uid);
...
var git;
(function (git) {
    function diffDashboards(existingDashboardPath, repoDashboardPath) {
        const command = 'git diff --no-index --ignore-all-space "'.concat(existingDashboardPath, '" "', repoDashboardPath, '" || true');
        const results = execCommand(command);
        return results.length > 0;
    }
    git.diffDashboards = diffDashboards;
})(git = exports.git || (exports.git = {}));
function execCommand(command) {
    try {
        return (0, child_process_1.execSync)(command, { stdio: ['pipe', 'pipe', 'ignore'] })
            .toString()
            .trim();
    }
    catch (ex) {
...
const sync_request_1 = __importDefault(__nccwpck_require__(937));
const util_1 = __nccwpck_require__(744);
const deployment_1 = __nccwpck_require__(292);
const check_1 = __nccwpck_require__(550);
var grafana;
(function (grafana) {
    ...
    function getFolderId(folderName) {
        const res = get('/api/folders');
        if (res.isError()) {
            util_1.util.reportAndFail('call to get folders failed', getResponseError(res));
        }
        const folders = JSON.parse(res.getBody('utf8'));
    ...
    function getDashboard(uid) {
        const res = get('/api/dashboards/uid/'.concat(uid));
        if (res.isError()) {
            if (res.statusCode === 404) {
                return undefined;
            }
            util_1.util.reportAndFail('call to get dashboard failed', getResponseError(res));
        }
    ...
    function createFolder(folderName) {
        ...
        const folder = JSON.parse(res.getBody('utf8'));
        return folder['id'];
    }
...
function get(path) {
    return (0, sync_request_1.default)('GET', 'http://'.concat(util_1.util.getGrafanaHost(), path), {
        headers: {
...
function post(path, data) {
    return (0, sync_request_1.default)('POST', 'http://'.concat(util_1.util.getGrafanaHost(), path), {
        headers: {
...
function getAuthorization() {
    return 'Basic '.concat(Buffer.from(util_1.util.getGrafanaAuthorization()).toString('base64'));
}
...
const util_1 = __nccwpck_require__(744);
const grafana_1 = __nccwpck_require__(907);
const commander_1 = __nccwpck_require__(662);
...
(function (util) {
    ...
    util.getGrafanaAuthorization = getGrafanaAuthorization;
...
exports.__esModule = true;
var GenericResponse = __nccwpck_require__(441);
var FormData_1 = __nccwpck_require__(78);
exports.FormData = FormData_1.FormData;
var init = __nccwpck_require__(189);
...
const JSON = __nccwpck_require__(836);
